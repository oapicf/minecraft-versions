...
mod paths {
    ...
    lazy_static! {
        pub static ref REGEX_V1_PACKAGES_PACKAGEID_VERSIONID_JSON: regex::Regex =
            ...
            regex::Regex::new(r"^/v1/packages/(?P<packageId>[^/?#]*)/(?P<versionId>[^/?#]*).json$")
                .expect("Unable to create regex for V1_PACKAGES_PACKAGEID_VERSIONID_JSON");
    }
}
    ...
    fn call(&self, req: (Request<ReqBody>, C)) -> Self::Future {
            ...
            ReqBody::Data: Send,
        {
            ...
            hyper::Method::GET if path.matched(paths::ID_MC_GAME_VERSION_MANIFEST_JSON) => {
                handle_get_minecraft_version_manifest(api_impl, uri, headers, body, context, validation).await
            },
            ...
            hyper::Method::GET if path.matched(paths::ID_V1_PACKAGES_PACKAGEID_VERSIONID_JSON) => {
                handle_get_minecraft_version_package_info(api_impl, uri, headers, body, context, validation).await
            },
...
async fn handle_get_minecraft_version_manifest<T, C, ReqBody>(
    mut api_impl: T,
    uri: hyper::Uri,
    headers: HeaderMap,
    body: ReqBody,
...
#[allow(unused_variables)]
async fn handle_get_minecraft_version_package_info<T, C, ReqBody>(
    mut api_impl: T,
    uri: hyper::Uri,
    headers: HeaderMap,
    body: ReqBody,
    ...
    ReqBody::Data: Send,
{
                ...
                let path_params =
                    paths::REGEX_V1_PACKAGES_PACKAGEID_VERSIONID_JSON
                    .captures(path)
                    .unwrap_or_else(||
                        panic!("Path {} matched RE V1_PACKAGES_PACKAGEID_VERSIONID_JSON in set but failed match against \"{}\"", path, paths::REGEX_V1_PACKAGES_PACKAGEID_VERSIONID_JSON.as_str())
                    );
                ...
                let param_package_id = match percent_encoding::percent_decode(path_params["packageId"].as_bytes()).decode_utf8() {
                    Ok(param_package_id) => match param_package_id.parse::<String>() {
                        ...
                        Err(e) => return Ok(Response::builder()
                                        .status(StatusCode::BAD_REQUEST)
                                        .body(body_from_string(format!("Couldn't parse path parameter packageId: {e}")))
                                        .expect("Unable to create Bad Request response for invalid path parameter")),
                    },
                    Err(_) => return Ok(Response::builder()
                                        .status(StatusCode::BAD_REQUEST)
                                        .body(body_from_string(format!("Couldn't percent-decode path parameter as UTF-8: {}", &path_params["packageId"])))
                                        .expect("Unable to create Bad Request response for invalid percent decode"))
                };

                let param_version_id = match percent_encoding::percent_decode(path_params["versionId"].as_bytes()).decode_utf8() {
                    Ok(param_version_id) => match param_version_id.parse::<String>() {
                        Ok(param_version_id) => param_version_id,
                        Err(e) => return Ok(Response::builder()
                                        .status(StatusCode::BAD_REQUEST)
                                        .body(body_from_string(format!("Couldn't parse path parameter versionId: {e}")))
                                        .expect("Unable to create Bad Request response for invalid path parameter")),
                    },
                    Err(_) => return Ok(Response::builder()
                                        .status(StatusCode::BAD_REQUEST)
                                        .body(body_from_string(format!("Couldn't percent-decode path parameter as UTF-8: {}", &path_params["versionId"])))
                                        .expect("Unable to create Bad Request response for invalid percent decode"))
                };
