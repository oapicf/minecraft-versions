...
${BOLD}${WHITE} command line client (API version 1.3.5-pre.0)${OFF}
...
echo -e "              \\t\\t\\t\\t(e.g. 'https://launchermeta.mojang.com')"
...
print_about() {
    echo ""
    echo -e "${BOLD}${WHITE} command line client (API version 1.3.5-pre.0)${OFF}"
    echo ""
    echo -e "License: MIT"
...
print_version() {
    echo ""
    echo -e "${BOLD} command line client (API version 1.3.5-pre.0)${OFF}"
    echo ""
}
...
call_getMinecraftVersionPackageInfo() {
    ...
    if ! path=$(build_request_path "/v1/packages/{packageId}/{versionId}.json" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
