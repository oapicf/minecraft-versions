...
async fn main() -> Result<()> {
    ...
    let result = match args.operation {
        Operation::GetMinecraftVersionManifest {
            ...
            info!("Performing a GetMinecraftVersionManifest request");

            let result = client.get_minecraft_version_manifest(
            ).await?;
            debug!("Result: {:?}", result);
        ...
        Operation::GetMinecraftVersionPackageInfo {
            ...
            info!("Performing a GetMinecraftVersionPackageInfo request on {:?}", (
                &package_id,
                &version_id
            ));

            let result = client.get_minecraft_version_package_info(
                package_id,
                version_id,
            ).await?;
            debug!("Result: {:?}", result);
