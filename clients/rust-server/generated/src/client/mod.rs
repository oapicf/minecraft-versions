    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
        ...
        context: &C) -> Result<McGameVersionManifestGetResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::McGameVersionManifestGet200Response>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(McGameVersionManifestGetResponse::AListOfMinecraftVersionsWithTheLatestAndSnapshotReleases
                    (body)
                )
            }
            code => {
        ...
        context: &C) -> Result<V1PackagesPackageIdVersionIdJsonGetResponse, ApiError>
    {
        let mut client_service = self.client_service.clone();
        let mut uri = format!(
            "{}/v1/packages/{package_id}/{version_id}.json",
            self.base_path
            ,package_id=utf8_percent_encode(&param_package_id.to_string(), ID_ENCODE_SET)
            ,version_id=utf8_percent_encode(&param_version_id.to_string(), ID_ENCODE_SET)
        );
