...
std::shared_ptr<VersionManifest>
DefaultApi::getMinecraftVersionManifest(
    ) {
    std::string serializedRequestBody;
    std::string path = m_context + "/mc/game/version_manifest.json";
    ...
    std::shared_ptr<VersionManifest> deserializedResponse = std::make_shared<VersionManifest>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<VersionManifest>>::deserialize(
...
std::shared_ptr<VersionPackageInfo>
DefaultApi::getMinecraftVersionPackageInfo(
    const std::string& packageId, const std::string& versionId) {
    std::string serializedRequestBody;
    std::string path = m_context + "/v1/packages/{packageId}/{versionId}.json";
    std::map<std::string, std::string> headers;
    // path params
    replacePathParameter(path, "packageId", packageId);
    replacePathParameter(path, "versionId", versionId);
    ...
    std::shared_ptr<VersionPackageInfo> deserializedResponse = std::make_shared<VersionPackageInfo>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<VersionPackageInfo>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
