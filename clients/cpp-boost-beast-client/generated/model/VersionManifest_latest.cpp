...
VersionManifest_latest::VersionManifest_latest(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value VersionManifest_latest::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void VersionManifest_latest::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for VersionManifest_latest");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object VersionManifest_latest::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_ReleaseIsSet) {
            object["release"] = JsonValueConverter<std::string>::toJsonValue(getRelease());
        }
        if (m_SnapshotIsSet) {
            object["snapshot"] = JsonValueConverter<std::string>::toJsonValue(getSnapshot());
        }
    return object;
}

void VersionManifest_latest::fromJsonObject_internal(boost::json::object const& object)
{
    m_ReleaseIsSet = false;
    m_SnapshotIsSet = false;
    {
        const auto ReleaseIt = object.find("release");
        if (ReleaseIt != object.end()) {
            setRelease(JsonValueConverter<std::string>::fromJsonValue(ReleaseIt->value()));
        }
    }
    {
        const auto SnapshotIt = object.find("snapshot");
        if (SnapshotIt != object.end()) {
            setSnapshot(JsonValueConverter<std::string>::fromJsonValue(SnapshotIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<VersionManifest_latest>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<VersionManifest_latest>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<VersionManifest_latest>>>::fromJsonValue(boost::json::parse(json));
}
