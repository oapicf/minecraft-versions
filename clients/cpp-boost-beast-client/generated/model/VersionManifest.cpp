...
VersionManifest::VersionManifest(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value VersionManifest::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void VersionManifest::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for VersionManifest");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object VersionManifest::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_LatestIsSet) {
            object["latest"] = JsonValueConverter<std::shared_ptr<VersionManifest_latest>>::toJsonValue(getLatest());
        }
        if (m_VersionsIsSet) {
            object["versions"] = JsonValueConverter<std::vector<std::shared_ptr<Version>>>::toJsonValue(getVersions());
        }
    return object;
}

void VersionManifest::fromJsonObject_internal(boost::json::object const& object)
{
    m_LatestIsSet = false;
    m_VersionsIsSet = false;
    {
        const auto LatestIt = object.find("latest");
        if (LatestIt != object.end()) {
            setLatest(JsonValueConverter<std::shared_ptr<VersionManifest_latest>>::fromJsonValue(LatestIt->value()));
        }
    }
    {
        const auto VersionsIt = object.find("versions");
        if (VersionsIt != object.end()) {
            setVersions(JsonValueConverter<std::vector<std::shared_ptr<Version>>>::fromJsonValue(VersionsIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<VersionManifest>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<VersionManifest>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<VersionManifest>>>::fromJsonValue(boost::json::parse(json));
}
