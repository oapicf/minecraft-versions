...
VersionPackageInfo_javaVersion::VersionPackageInfo_javaVersion(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value VersionPackageInfo_javaVersion::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void VersionPackageInfo_javaVersion::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for VersionPackageInfo_javaVersion");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object VersionPackageInfo_javaVersion::toJsonObject_internal() const
{
    boost::json::object object;
        if (m_ComponentIsSet) {
            object["component"] = JsonValueConverter<std::string>::toJsonValue(getComponent());
        }
        if (m_MajorVersionIsSet) {
            object["majorVersion"] = JsonValueConverter<int32_t>::toJsonValue(getMajorVersion());
        }
    return object;
}

void VersionPackageInfo_javaVersion::fromJsonObject_internal(boost::json::object const& object)
{
    m_ComponentIsSet = false;
    m_MajorVersionIsSet = false;
    {
        const auto ComponentIt = object.find("component");
        if (ComponentIt != object.end()) {
            setComponent(JsonValueConverter<std::string>::fromJsonValue(ComponentIt->value()));
        }
    }
    {
        const auto MajorVersionIt = object.find("majorVersion");
        if (MajorVersionIt != object.end()) {
            setMajorVersion(JsonValueConverter<int32_t>::fromJsonValue(MajorVersionIt->value()));
        }
    }
}
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<VersionPackageInfo_javaVersion>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<VersionPackageInfo_javaVersion>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<VersionPackageInfo_javaVersion>>>::fromJsonValue(boost::json::parse(json));
}
